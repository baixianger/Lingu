...
enum KeychainHelper {
    private static let service = "me.pai.lingu"
    ...
    static func setAPIKey(_ value: String, for provider: TranslationProvider) {
        if value.isEmpty {
            delete(key: "apiKey_\(provider.rawValue)")
        } else {
            save(key: "apiKey_\(provider.rawValue)", value: value)
        }
    }
}
